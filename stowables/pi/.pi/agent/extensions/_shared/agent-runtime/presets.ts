...
// Pi loads extensions through jiti, which has no bundler text imports; read the
// prompt files relative to this module instead.
const agentsDir = path.join(path.dirname(fileURLToPath(import.meta.url)), "agents");
const explorePrompt = fs.readFileSync(path.join(agentsDir, "explore.md"), "utf8");
const workerPrompt = fs.readFileSync(path.join(agentsDir, "worker.md"), "utf8");
